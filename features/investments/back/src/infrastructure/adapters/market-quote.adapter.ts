...
export type GetQuotesFn = (symbols: string[]) => Promise<Array<{ symbol: string; price?: number }>>;
...
export class MarketQuoteAdapter implements GetQuotePort {
  ...
  async getQuote(symbol: string): Promise<{ price: number; timestamp?: Date } | null> {
    const normalized = symbol?.trim().toUpperCase();
    if (!normalized) return null;
    const quotes = await this.getQuotes([normalized]);
    const item = quotes.find((q) => (q.symbol ?? '').toUpperCase() === normalized);
