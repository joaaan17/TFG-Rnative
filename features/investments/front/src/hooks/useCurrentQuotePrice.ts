...
export function useCurrentQuotePrice(symbol: string, enabled: boolean) {
  ...
  useEffect(() => {
    ...
    fetchPrice();
    const intervalId = setInterval(fetchPrice, REFRESH_INTERVAL_MS);
    return () => clearInterval(intervalId);
  }, [enabled, symbol, fetchPrice]);
